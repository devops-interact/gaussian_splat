...
- **Measurement tool** — click two points (A = lavender, B = green); **mousemove** shows a preview on the picked surface. Optional **wireframe mesh picks** (checkbox, default on): a **mid-poly** proxy mesh is built client-side from the splat **world center cache** ([`measureOverlayMesh.ts`](frontend/src/lib/measureOverlayMesh.ts): voxel downsample toward **~3k** cells, **PCA** best-fit plane then **Delaunay** + long-edge cull, with **bbox-axis projection** fallback when the cloud is too isotropic), drawn as **cyan wireframe** with **polygon offset** scaled by **`VITE_VIEWER_SCENE_SCALE`** to reduce z-fight. When enabled, picks use **Three.js `Raycaster` on that mesh only** (no GS3D splat raycast / center-cache / ground fallback). Uncheck to fall back to splat picking ([`splatPick.ts`](frontend/src/lib/splatPick.ts)). See [`docs/measure-mesh-overlay-prompt.md`](docs/measure-mesh-overlay-prompt.md).
