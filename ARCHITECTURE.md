...
- **Measurement tool** — click two points (A = lavender, B = green); **mousemove** shows a preview on the picked surface. Optional **wireframe mesh picks** (checkbox, default on): a **low-poly** proxy mesh is built client-side from the splat **world center cache** ([`measureOverlayMesh.ts`](frontend/src/lib/measureOverlayMesh.ts): voxel downsample + planar **Delaunay** + long-edge cull), drawn as **cyan wireframe** with **polygon offset** to reduce z-fight. When enabled, picks use **Three.js `Raycaster` on that mesh only** (no GS3D splat raycast / center-cache / ground fallback). Uncheck to fall back to splat picking ([`splatPick.ts`](frontend/src/lib/splatPick.ts)). See [`docs/measure-mesh-overlay-prompt.md`](docs/measure-mesh-overlay-prompt.md).
