...
- **Orbit mode** — rotate, pan, zoom with OrbitControls. After **`viewer.start()`**, `Viewer3D` patches **`minDistance`** / **`maxDistance`** from the PLY bbox diagonal × **`VITE_VIEWER_SCENE_SCALE`** (defaults **1**) so scroll-zoom can move closer than GS3D defaults. Optional **`VITE_VIEWER_SCENE_SCALE`** (clamped **0.25–10**) applies **`splatMesh.scale`**, scales initial camera eye positions from the look-at target, and scales measure **`maxDist`**. **`GET /api/jobs/{id}/initial_camera`** places the eye at **`cameras_all.json[0]`**’s camera center (PLY-centered frame; index **0** matches sorted extracted frames in training). **`target`** lies **`forward × (bbox_diagonal × frac)`** along that pose’s view axis (**`INITIAL_CAMERA_DIAGONAL_FRAC`** default **0.6**, overridable with **`INITIAL_CAMERA_DISTANCE_FRAC`** **0.35–0.95** on the API host); see [`viewer_initial_camera.py`](backend/services/viewer_initial_camera.py).
...
- **Display panel** — Min alpha (debounced → scene reload), SH 0/1/2 (live), **Download .ksplat** (client-side), and a short cross-origin isolation hint. Splat ellipsoid scale is fixed at Viewer init (**~1.25**, `DEFAULT_SPLAT_SCALE` in `Viewer3D.tsx`) — no live slider in the UI.
